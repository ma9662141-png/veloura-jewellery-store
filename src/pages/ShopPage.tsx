...
export default function ShopPage() {
  ...
  const filtered = useMemo(() => {
    let items = (productsData || []).filter((p: any) => p.status === 'active');
    if (selectedCategories.length > 0) {
      items = items.filter((p: any) => p.category && selectedCategories.includes(p.category.slug));
    }
    items = items.filter((p: any) => p.price >= priceRange[0] && p.price <= priceRange[1]);
    if (onSaleOnly) items = items.filter((p: any) => p.is_on_sale);
    ...
    return items;
  }, [selectedCategories, priceRange, sortBy, onSaleOnly]);
    ...
    <div className="space-y-6">
      ...
      {(selectedCategories.length > 0 || onSaleOnly) && (
        <Button
          variant="ghost"
          size="sm"
          onClick={() => { setSelectedCategories([]); setOnSaleOnly(false); setPriceRange([0, 5000]); }}
