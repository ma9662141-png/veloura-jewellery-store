...
export default function CartPage() {
  ...
  const [standardFee, setStandardFee] = useState(150);
  const [whatsappNumber, setWhatsappNumber] = useState('+923075323246');

  useEffect(() => {
    supabase
      .from('store_settings')
      .select('free_delivery_above, standard_delivery_fee, whatsapp_number')
      .eq('id', 1)
      .single()
      .then(({ data }) => {
        if (data) {
          setFreeAbove(data.free_delivery_above);
          setStandardFee(data.standard_delivery_fee);
          if (data.whatsapp_number) setWhatsappNumber(data.whatsapp_number);
        }
      });
  }, []);

  const deliveryFee = subtotal >= freeAbove ? 0 : standardFee;
  const total = subtotal + deliveryFee;

  // Normalise phone: strip non-digits, convert leading 0 to country code 92
  ...
  const cartMsg = items
    .map((i) => `🛍️ ${i.product.name} x${i.quantity} — Rs. ${(i.product.price * i.quantity).toLocaleString()}`)
    .join('\n');
                  ...
                  <div key={item.id} className="flex gap-4 rounded-xl bg-card p-4 shadow-sm">
                    ...
                    <div className="flex flex-1 flex-col justify-between">
                      <div>
                        ...
                        {item.variant && (
                          <p className="mt-0.5 font-body text-xs text-muted-foreground">{item.variant.name}</p>
                        )}
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 rounded-xl bg-card p-6 shadow-sm">
              ...
              <div className="space-y-3 font-body text-sm">
                ...
                {subtotal < freeAbove && (
                  <p className="rounded-lg bg-primary/5 p-2 text-center text-xs text-muted-foreground">
                    Add Rs. {(freeAbove - subtotal).toLocaleString()} more for{' '}
