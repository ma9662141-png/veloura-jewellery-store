...
import { STORE_SETTINGS as FALLBACK_SETTINGS } from '@/data/mock';
...
export default function HomePage() {
  const { data: bestSellers = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('products').select('*, categories(name), product_images(url, is_main)').eq('is_featured', true).eq('status', 'active').limit(8);
  ...
  const { data: reviews = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('reviews').select('*, profiles(full_name, city)').eq('is_visible', true).limit(3);
      return (data || []).map((r: any) => ({
        ...r,
        user: r.profiles || { full_name: 'Customer' }
      }));
    }
  });
  ...
  const STORE_SETTINGS = storeSettings || FALLBACK_SETTINGS;
  const heroImage = STORE_SETTINGS.hero_image_url || FALLBACK_HERO;
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {reviews.map((review) => (
              ...
              >
                ...
                <p className="mb-4 font-body text-sm text-muted-foreground">{review.body || review.comment}</p>
                <div className="font-body text-xs">
                  <span className="font-medium">{review.user?.full_name}</span>
                  {review.user?.city && <span className="text-muted-foreground"> · {review.user.city}</span>}
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="bg-primary py-20">
        <div className="container text-center">
          ...
          <div className="mx-auto mt-8 flex max-w-md gap-3">
            ...
            />
            <Button variant="secondary" size="lg" className="shrink-0 rounded-full font-body">
